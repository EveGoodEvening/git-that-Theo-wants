...
- [ ] Define `SecretBlob { id, ciphertext, policyId }` in `src/core/secret-blob.ts`; `ciphertext` is **self-describing framed** as `version(1) || algId(1) || iv(12) || ciphertext(N) || tag(16)` (iv + GCM auth tag persisted inside the bytes; no external IV field); serializes into `ContentObject` envelope `kind: 'secret-blob'` (envelope defined in C1; C5 imports it, no C1 edits); stored via `Store.putObject`
- [ ] Implement `encryptSecret`/`decryptSecret` (Web Crypto AES-GCM, random 12-byte IV, 16-byte auth tag, local key stub); `encryptSecret` produces framed `version||algId||iv||ciphertext||tag` bytes; `decryptSecret` parses framing, extracts iv+tag, verifies GCM auth tag (tag mismatch → typed `Denied`)
- [ ] Bind access policy to signed ACL graph (C1), not user-editable config
- [ ] Key rotation stub: new policyId under new key for new content
- [ ] Tests (low-level crypto+store, NOT commit/snapshot — C5 does not require C3/C4): `encryptSecret` → ciphertext (no plaintext) → `Store.putObject` → `Store.getObject` returns same ciphertext → `decryptSecret` with authorized key → plaintext; unauthorized key → typed `Denied`, no plaintext; rotate key → old key cannot decrypt new content. **Framing/round-trip-from-stored-bytes tests**: framed `ciphertext` parses to recorded version/alg/iv/tag; `decryptSecret` succeeds using **only** `getObject` bytes (no out-of-band IV); two encryptions of same plaintext under same key → different iv/ciphertext (random IV); tampered tag byte → `Denied` (auth-tag integrity); key-rotation denial survives store round-trip
...
- [ ] Define visibility states `public | private | embargoed | local-only` (file + snapshot/ref level) AND the **state-operation matrix** (no time-based behavior): `public` → readable by other peer + present in export; `private` → authorized owner reads, other peer `Denied`, omitted from export, `publish`→`public`; `embargoed` → owner reads, other peer `Denied`, omitted from export, `publish`→`public` (explicit only, **no time-based auto-release**); `local-only` → owner-local reads, other peer `Denied`, omitted from export, `publish`/`export` rejected
- [ ] Implement deterministic authorization checks (read/checkout/publish)
- [ ] Implement `publish` transition (private/embargoed → public, op-log recorded; no time side channel)
- [ ] Implement `unpublish` (re-privatization): public → private as a NEW op-log event (§2 decision 7); flips visibility for future readers; cannot recall already-exported content; op-log stays append-only
- [ ] Define public manifest with explicit schema: `{ bundleVersion:1, publicProjectionIds: PublicProjectionId[], publicEntries[{path,blobId}], publicManifestHash }`. `publicManifestHash` is a **deterministic self-hash**: computed over the canonical manifest payload with the `publicManifestHash` field itself **omitted** (equivalently, set to a fixed `null`/zero placeholder before hashing, then written into that field afterward) — this makes it reproducible by a verifier that does not trust the stored value. `PublicProjectionId` = `hash(bundleVersion || canonical(publicEntries) || canonical(publicProjectionIds-of-nearest-public-visible-ancestors))` — parent ids are the **nearest public-visible ancestor** projection ids, computed by eliding private-only and public-noop snapshots from the parent chain (public-noop = public entries + public-visible parents unchanged from predecessor); when a snapshot's public entries + public-visible parents are unchanged from its nearest public-visible ancestor, it **reuses that ancestor's `PublicProjectionId`**. Derived **only** from public entries/public metadata — never full private `SnapshotId` values (which embed timestamps/messages/private paths/private blob ids; `SnapshotId` excludes manifest refs per §2 decision 10). Private manifest (separate) holds private counterparts + the `SnapshotId → PublicProjectionId` mapping; since `SnapshotId` is final before the manifest is built, writing the manifest hash back into `privateManifestRef` does not change the `SnapshotId` — the mapping is **acyclic** (no fixed point)
- [ ] **Populate C4 snapshot manifest refs from C6-owned code**: C6 owns the concrete `PublicManifest`/private-manifest schemas + build/load/export logic; after building a manifest, compute its content hash, call C4's exported `Snapshot.withManifestRefs(public, private)` helper to produce a same-id snapshot value, then persist the refs by upserting Store's separate `ManifestRefs` attachment keyed by `SnapshotId` (not by replacing `SnapshotEnvelope.serializedBytes`) — all from `src/policy/`/`src/export/` code. C6 **must not edit** any C4-owned file (`src/snapshot/snapshot.ts`, `src/snapshot/bookmark.ts`, `src/snapshot/oplog.ts`, `src/workspace/working-copy.ts`); it only imports the helper and uses the C2 attachment API. Executable after C4 lands with no C4-file edit
- [ ] Tests: private PR → public peer sees only public subset → publish → visibility flips (no re-clone) → unpublish → future public peer sees private again, op-log has two events; publish-without-transition rejected; **state-operation matrix tests (file + snapshot/ref level, every state)**: public readable+exported; private authorized-only+omitted from export; embargoed public-denied until explicit publish with **no time behavior** (mock clock advance does not change visibility); local-only owner-local+rejected from publish/export; **metadata-absence tests (each class)**: zero private bytes, paths, blob/secret ids, manifest refs, sizes, timestamps, op-log entries, messages, **and zero full `SnapshotId` values** (only `PublicProjectionId`s appear); public-projection determinism + parent-leak/private-history-elision tests; bundle integrity + deterministic `publicManifestHash`; **manifest-ref persistence test:** build final `SnapshotId`, build manifests, write refs with `Snapshot.withManifestRefs`, upsert Store `ManifestRefs`, assert same `SnapshotId`, unchanged `SnapshotEnvelope.serializedBytes`, acyclic `SnapshotId → PublicProjectionId` mapping, and idempotent re-build/upsert
...
- [ ] **C8 status: not started** — authoritative skip marker (see plan.md §8). Change to `[~] **C8 status: skipped**` to intentionally skip C8 and use the C6 public-bundle fallback; change to `[x] **C8 status: complete**` when landed. This single status line controls C8's skipped-dependency/resume semantics for all downstream chunks; the individual C8 task boxes below remain `[ ]` (or `[~]` per chosen protocol) and do not separately gate resume.
